...
<head>
    ...
    <style>
        ...
        .title {
            ...
            text-align: center;
            width: 100%;
            text-shadow: 10px 8px 4px rgba(0, 0, 0, 6.6);
        }
        .login-container {
            ...
            margin-top: 20px; /* Espaçamento para evitar sobreposição com o título */
        }
        .login-container h3 {
            display: none; /* Oculta o título dentro da caixa */
        }
        .form-control {
            ...
            width: 100%; /* Garante que o campo ocupe o espaço total */
        }
        .form-control:focus {
            outline: none;
            border-color: #10b981;
        ...
        .btn-login {
            ...
            font-weight: 500;
            transition: all 0.3s ease;
            width: 100%; /* Mesmo tamanho do botão */
        }
        .btn-login:hover {
...
<body>
    <h1 class="title">CHECKER AMAZON</h1>
    <div class="login-container">
        <?php if (isset($erro)): ?>
            <div class="alert"><?php echo htmlspecialchars($erro); ?></div>
        <?php endif; ?>
        <form method="POST">
            <input type="text" name="usuario" class="form-control mb-7" placeholder="Usuário" required />
            <input type="password" name="senha" class="form-control mb-7" placeholder="Senha" required />
    ...
    <script>
        $(document).ready(function() {
            ...
            <?php if (!$_SESSION['is_admin'] && isset($_SESSION['username'], $users[$_SESSION['username']])): ?>
                const expirationTime = <?php echo strtotime($users[$_SESSION['username']]['expiration']) * 1000; ?>; // Em milissegundos
...
// Bloqueia teclas comuns para inspeção (F12, Ctrl+Shift+I, Ctrl+U, etc)
